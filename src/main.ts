...
/// <reference path="../node_modules/vite-userscript-plugin/types/tampermonkey.d.ts" />
...
function fetchMessages(region: string) {
...
function sendMessage(uid: string, name: string, message: string, region: string) {
    return new Promise<any>((resolve, reject) => {
        GM_xmlhttpRequest({
            method: 'POST',
            url: `${API_BASE}/send`,
...
const closeButton = modal.querySelector('.livechat-close') as HTMLButtonElement;
const userInfo = document.getElementById('userInfo') as HTMLElement;
const chatTabs = document.getElementById('chatTabs') as HTMLElement;
